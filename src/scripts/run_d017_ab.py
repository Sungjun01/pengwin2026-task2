...
os.environ["PIVOT_PELVIC_TRAINER_PLANS"] = "nnUNetTrainerBorderWeightedAseg_w40__nnUNetResEncUNetMPlans__3d_fullres"
...
OUT = Path("/tmp/v15_d017/out"); OUT.mkdir(parents=True, exist_ok=True)
...
for cid in ["003", "017", "025"]:
    ...
    print(f"=== D017 pipeline PENGWIN_{cid} (exists={inp.exists()}) ===", flush=True)
    run_pipeline_v7(input_ct=inp, output_path=outp, nnunet_results=RES,
                    tile_step_size=1.0, use_pivot_form=False, use_d017_sacrum=True,
...
print(f"MEAN(D017): LDSC={np.mean(ld):.3f} RQ={np.mean(rq):.3f}", flush=True)
print("D017_AB_DONE", flush=True)
